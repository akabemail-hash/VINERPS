...
export const Accounting = () => {
  ...
  const calculateAccountBalance = (account: Account, start: string, end: string): number => {
      ...
      if (account.systemLink === 'INVENTORY') {
          ...
          // Since our transaction model doesn't explicitly store cashRegisterId yet (simplification in types), 
          // we assume generic source='CASH_REGISTER'. 
          // In a full implementation, transactions need a `cashRegisterId` field.
          // For now, if we had it:
          /*
          const regTrans = transactions.filter(t => t.source === 'CASH_REGISTER' && t.cashRegisterId === account.systemLinkId && t.date <= end);
          ...
          */
          // Placeholder logic returning 0 if no specific data, or generic cash if undefined
          if (account.systemLinkId) {
             // Mock calculation assuming we might add register ID to transactions later
             // For now, return 0 to avoid double counting if parent is CASH
             balance = 0; 
          } else {
             balance = 0;
          }
  ...
  const AccountTreeItem = ({ account, depth = 0, isReport = false }: { account: Account, depth?: number, isReport?: boolean }) => {
