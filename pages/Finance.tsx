                      ...
                      {filteredTransactions.length === 0 ? (
                          ...
                          filteredTransactions.map(transaction => (
                              <tr key={transaction.id} className="hover:bg-gray-50 transition-colors">
                                  ...
                                  <td className="p-4 text-sm font-medium text-gray-700">{transaction.category}</td>
                ...
                <div className="space-y-4">
                    ...
                    {selectedPartnerId && (
                        <div>
                            ...
                            <select className="w-full border p-2 rounded focus:ring-2 focus:ring-primary outline-none" value={linkedInvoiceId} onChange={e => setLinkedInvoiceId(e.target.value)}>
                                <option value="">{t('linkNone')}</option>
                                {invoices.filter(i => i.partnerId === selectedPartnerId).map(inv => (
                                    <option key={inv.id} value={inv.id}>#{inv.id.slice(-6)} - {settings.currency}{inv.total}</option>
                                ))}
                            </select>
                        </div>
                    )}
